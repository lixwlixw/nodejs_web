...
$(function(){
    ...
    var thisstr = '<div class="commentwrop" id="replyCommnet">'+
                  '<textarea name="" id="replycommentcon"></textarea>'+
                  '<div class="conmentbt">'+
                  '<div class="commentnums">还可以输入<span class="reply_surplusnum">210</span>字，已超出<span class="reply_exceednum">50</span>字</div>'+
                  '<div class="publish_btn" id="replycon_btn">发表评论</div>'+
                  '</div>'+
                  '</div>';
    ////////////////////////////////////////生成uuid
    function S4() {
    ...
    function addreplyhtml(listcon){
        var aplystr = '<div class="comListconwrop" datacomid="'+listcon.commentid+'">'+
                      '<div class="comnews">'+listcon.content+'</div>'+
                      '<span class="commenname">'+listcon.username+'</span>'+
                      '<p class="commentdate">'+listcon.createtime+'</p>'+
                      '<div class="reply_wrop"><span class="reply_btn">回复</span></div>'+
                      '</div>';
    ...
    $.ajax({
        type:'GET',
        url: ngUrl + "/comments/" + repoName + "/"+itemName,
        cache: false,
        dataType:'json',
        success: function (msg) {
            var msglenth = msg.data.length;
    ...
    $(document).on('click','.reply_btn',function(){
        ...
        if(thistext == '回复'){
            ...
            $(this).parents('.comListconwrop').append(thisstr);
        }else if(thistext == '收起'){
            $(this).html('回复');
            $(this).parent().siblings('#replyCommnet').remove();
    }
    ...
    $('#commentcon').keyup(function(){
        ...
        $('.surplusnum').html(residue);
        $('.exceednum').html(exceeding);
